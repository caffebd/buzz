...
@onready var gun_pos := %BulletMarker
...
func _reward_over():
	$Footsteps.stop()
	$CanvasLayer/SubViewportContainer.mouse_filter = 2
	all_over = true
...
func _fire():
	if all_over:
		return
	if get_parent().ammo_left > 0:
		%WeaponShoot.play()
		GlobalSignals.emit_signal("ammo_count", -1)
